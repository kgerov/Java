...
public class FoxAndHounds {
	...
	public static void main(String[] args) {
		...
		while(true) {
			...
			do {
				...
				xf = inp.nextInt();
				yf = inp.nextInt();
			} while (validFoxMove(table, xf, yf, xFoxCurr, yFoxCurr) == false); //loop until a valid move is entered for the fox
			...
			do {
				...
				xh = inp.nextInt();
				yh = inp.nextInt();
			} while (validHoundMove(table, xh, yh, xhCurr, yhCurr) == false); //Check if it possible to move the hound to this position
	...
	private static boolean validHoundMove(char[][] table, int xh, int yh, int xhCurr, int yhCurr) { 
		boolean isValid = false;

        if ( (xh >= 0 && xh < 8) && (yh >= 0 && yh < 8) ) {
            if ( xh == (xhCurr + 1) && (yh == yhCurr + 1) && table[xh][yh] == '-' ) {
                isValid = true;
            }
            else if ( xh == (xhCurr + 1) && (yh == yhCurr - 1) && table[xh][yh] == '-' ) {
                isValid = true;
            }
            else {
           	  isValid = false;
	...
	private static boolean validFoxMove(char[][] table, int xf, int yf, int xFoxCurr, int yFoxCurr) {
		boolean isValid = false;
        if ( (yf < 8 && yf >= 0) && (xf >= 0 && xf < 8) ) {
            if ( (xf == xFoxCurr + 1) && (yf == yFoxCurr + 1) && table[xf][yf] == '-' ) {
                isValid = true;
            }
            else if ( (xf == xFoxCurr + 1) && (yf == yFoxCurr - 1) && table[xf][yf] == '-' ) {
                isValid = true;
            }
            else if ( (xf == xFoxCurr - 1) && (yf == yFoxCurr + 1) && (table[xf][yf] == '-') ) {
                isValid = true;
